...
class TestMessage:
    ...
    def test_message_with_all_fields(self):
        ...
        token_usage = TokenUsage(
            prompt_tokens=10, completion_tokens=20, total_tokens=30, estimated_cost=0.001
        )
        msg = Message(
...
class TestModelConfig:
    ...
    def test_model_config_api_key_excluded(self):
        ...
        config = ModelConfig(
            provider=ModelProvider.OPENAI, model_name="gpt-4", api_key="secret-key"
        )
    ...
    def test_model_config_validation(self):
        ...
        with pytest.raises(ValueError):
            ModelConfig(provider=ModelProvider.OPENAI, model_name="gpt-4", temperature=3.0)

        # Negative max_tokens
        with pytest.raises(ValueError):
            ModelConfig(provider=ModelProvider.OPENAI, model_name="gpt-4", max_tokens=-100)
...
class TestChatSession:
    ...
    def test_get_messages_for_model(self):
        ...
        model1_msg = Message(
            role=MessageRole.ASSISTANT, content="Hi from model1", model_id="model1"
        )
        model2_msg = Message(
            role=MessageRole.ASSISTANT, content="Hi from model2", model_id="model2"
        )
