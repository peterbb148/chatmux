...
class InputArea(TextArea):
    """Custom TextArea that detects Cmd/Ctrl+Enter."""
    
    # Completely override bindings to prevent conflicts
    BINDINGS = [
        B("escape", "escape", "", show=False),  # Allow escape to bubble up
    ]
    
    async def _watch_key_press(self, event: events.Key) -> None:
        """Watch for specific key combinations before TextArea processes them."""
        # Check if this is our special combination
        if event.key == "ctrl+m":  # This is often how Ctrl+Enter appears
            app = self.app
            if app and hasattr(app, 'action_send_message'):
                ...
                return
        # Otherwise let TextArea handle it normally
        await super()._watch_key_press(event)
...
class ModelPane(Static):
    ...
    def compose(self) -> ComposeResult:
        ...
        yield TextArea(id="content", classes="pane-content", read_only=True, show_line_numbers=False)
...
class ChatmuxTextualApp(App):
    ...
    BINDINGS = [
        ...
        Binding("cmd+return", "send_message", "Send message", priority=True),
        Binding("ctrl+return", "send_message", "Send message", priority=True),
    ]
    ...
    async def on_key(self, event) -> None:
        """Capture keys at app level before widgets see them."""
        # Check if the input is focused
        try:
            input_area = self.query_one("#main-input", InputArea)
            if self.focused == input_area:
                # Check for cmd+enter or ctrl+enter
                if event.key in ("ctrl+m", "ctrl+j") or \
                   (event.key == "enter" and hasattr(event, 'modifiers') and 
                    ('ctrl' in str(event.modifiers).lower() or 'cmd' in str(event.modifiers).lower())):
    ...
    def compose(self) -> ComposeResult:
        ...
        with Vertical():
            ...
            yield Button("Send [S]", id="send-button", variant="primary")
    ...
    def on_mount(self) -> None:
        ...
        import platform
        key_combo = "Cmd+Enter" if platform.system() == "Darwin" else "Ctrl+Enter"
        input_area.border_subtitle = f"{key_combo} to send | @model to target"
    ...
    async def action_copy_focused(self) -> None:
        ...
        if focused and hasattr(focused, 'pane_id'):
            ...
            try:
                content = focused._accumulated_content if hasattr(focused, '_accumulated_content') else ""
                if content:
                    pyperclip.copy(content)
                    self.notify(f"Copied {len(content)} characters to clipboard!", severity="information")
                else:
            ...
            for pane_id, pane in self.model_panes.items():
                if pane.has_focus or any(child.has_focus for child in pane.walk_children()):
                    try:
                        content = pane._accumulated_content if hasattr(pane, '_accumulated_content') else ""
                        if content:
                            pyperclip.copy(content)
                            self.notify(f"Copied {len(content)} characters to clipboard!", severity="information")
                        else:
